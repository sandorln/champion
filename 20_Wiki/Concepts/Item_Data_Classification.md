...
4. **칼바람 나락 전용 수호자 시작 아이템 (`ARAM_ONLY_ITEM_ID_SET`)**:
   - [EXTRACTED] 수호자의 뿔피리(`2051`, Patch 3.07/시즌 3), 수호자의 보주(`3112`, Patch 6.12), 수호자의 망치(`3184`, Patch 6.12), 수호자의 검(`3177`, Patch 10.23)은 정식 출시 시점부터 칼바람 나락(ARAM) 전용 시작 아이템이다.
   - [EXTRACTED] 라이엇 Data Dragon 메타데이터 상 `maps["11"] = true, maps["12"] = true`로 잘못 설정되어 있어 소환사의 협곡 목록에 노출되는 문제가 존재한다.
   - [INFERRED] 해당 4종 아이템은 `MapType.ARAM`으로 명시적 강제 분류하여 협곡 목록에서 제외하고 칼바람 나락 전용으로 격리한다.
