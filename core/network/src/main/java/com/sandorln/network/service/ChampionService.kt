    ...
    suspend fun getChampionPathNoteList(version: String): List<NetworkPatchNoteData> = withContext(Dispatchers.IO) {
        ...
        val oldPatchUrl = async {
            runCatching {
                ...
                Jsoup.connect(url).get().toNetworkPatchNoteList(NetworkPatchNoteType.Champion)
            }
        }
        ...
        val finalChampionPatchList = oldUrlChampionResult?.takeIf(List<NetworkPatchNoteData>::isNotEmpty) ?: newUrlChampionResult ?: emptyList()
